...
改修や実装の依頼を受けたら、コードを変更する前に必ず現在のファイル状況および Git ブランチを確認する。

1. **未コミットの変更・編集途中のファイルがある場合**
   - 直ちに作業を停止し、どう進めるべきか（変更を破棄するか、退避するか、現在のブランチで継続するか等）をユーザーへ確認する。
2. **編集途中のファイルがない（クリーンな）場合**
   - まず現在のブランチを確認する。
   - **`main` ブランチの場合**: `git pull origin main` を実行して最新状態にしてから進める。
   - **`main` 以外のブランチの場合**:
